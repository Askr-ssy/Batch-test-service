...
def run(*args, **kwargs):
    ...
    sql="SELECT `id` FROM `emotion_data` where userid='{}'".format(context["data"]["user_id"])
    try:
        ...
        # datas=datas[0]
    # 映射id 待优化 通用型
    datas_list=[value[0] for index,value in enumerate(datas) if index%context["data"]["sub_count"]==uid]    # 根据uid分配测试数据条例
    ...
    for data in datas:
        ...
        try:
            time.sleep(0.01)
            response = requests.post(url=u_config.NLP_URL, data=request_data, headers=u_config.NLP_HEADERS)
            ...
            # 优化error
            continue
        r.conn.hset(context["data"]["task_name"],data[0],json.dumps(sentiment))
        Result[0].append(label)    # 结果聚合待优化通用
        Result[1].append(data[1] if ""!=data[1] else data[-1])
        Result[2].append(data[0])
    r.conn.expire(context["data"]["task_name"],86400)  # 定时一周
